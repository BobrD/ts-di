...
export interface FactoryBuilderInterface<T> {
    createFactory(definition: Definition, containerBuilder: ContainerBuilder): Promise<() => T>;
}

export class ClassFactoryBuilder<T> implements FactoryBuilderInterface<T> {

    async createFactory(definition: Definition, containerBuilder: ContainerBuilder): Promise<() => T> {
